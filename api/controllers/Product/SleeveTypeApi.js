const SleeveTypeModel = require("../../../api/models/Product/SleeveType");
const crudService = require("../../services/crud.service");
const validationService = require("../../services/validation.service");
const { SleeveTypeSchemas } = require("../../schemas/ProductSchemas");
const Sequelize = require("sequelize");
const { Op } = require("sequelize");
const fireBase = require("../../helper/firebaseHelper");
const SleeveTypeApi = () => {
  // For create and update Sleeve Type
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, SleeveTypeSchemas)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData.id) {
            ...
            response = await crudService.insert(SleeveTypeModel, reqData);
          }
    ...
    try {
      if (req.body.record_id) {
        for (let id of req.body.record_id) {
          let response = await crudService.destroy(SleeveTypeModel, { id: id });
        }
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Sleeve Type deleted successfully.`,
          data: {},
        });
      } else {
  ...
  const get = async (req, res) => {
    try {
      ...
      response = await crudService.getAll(SleeveTypeModel, {
        where: whereClause,
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: `Sleeve Type get successfully.`,
        data: response.rows,
        page_info: page_info,
      });
    } catch (error) {
