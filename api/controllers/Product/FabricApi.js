const FabricModel = require("../../../api/models/Product/Fabric");
const crudService = require("../../services/crud.service");
const validationService = require("../../services/validation.service");
const { FabricSchemas } = require("../../schemas/ProductSchemas");
const Sequelize = require("sequelize");
const { Op } = require("sequelize");
const fireBase = require("../../helper/firebaseHelper");
const FabricApi = () => {
  // For create and update Fabric
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, FabricSchemas)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData.id) {
            ...
            response = await crudService.insert(FabricModel, reqData);
          }
    ...
    try {
      if (req.body.record_id) {
        for (let id of req.body.record_id) {
          let response = await crudService.destroy(FabricModel, { id: id });
        }
  ...
  const get = async (req, res) => {
    try {
      ...
      response = await crudService.getAll(FabricModel, {
        where: whereClause,
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: `Fabric get successfully.`,
        data: response.rows,
        page_info: page_info,
      });
    } catch (error) {
