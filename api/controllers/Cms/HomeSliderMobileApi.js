const HomeSliderMobileModel = require("../../models/Cms/HomeSliderMobile");
const crudService = require("../../services/crud.service");
const Sequelize = require("sequelize");
const { Op } = require("sequelize");
const fireBase = require("../../helper/firebaseHelper");
const { HomeSliderSchemasMobile } = require("../../schemas/CmsSchemas");
const validationService = require("../../services/validation.service");
const Collection = require("../../models/Product/Collection");

const HomeSliderForMobileApi = () => {
  ...
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, HomeSliderSchemasMobile)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData.id) {
            response = await crudService.update(
              HomeSliderMobileModel,
              { id: reqData.id },
              reqData
            );
            response = reqData;
          } else {
            response = await crudService.insert(HomeSliderMobileModel, reqData);
          }
    ...
    try {
      if (req.body.record_id) {
        for (let id of req.body.record_id) {
          let response = await crudService.destroy(HomeSliderMobileModel, {
            id: id,
          });
        }
  ...
  const get = async (req, res) => {
    try {
      ...
      response = await crudService.getAll(HomeSliderMobileModel, {
        where: whereClause,
...
module.exports = HomeSliderForMobileApi;
