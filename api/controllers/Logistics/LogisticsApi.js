...
const { LogisticsSchemas } = require("../../schemas/LogisticsSchemas");
const Sequelize = require("sequelize");
const { Op } = require("sequelize");
const Logistics = require("../../models/Logistics/Logistics");
const LogisticsApi = () => {
  // For create and update Logistics
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);

    validationService
      .validate(req.body, LogisticsSchemas)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData.id) {
            ...
            response = await crudService.insert(Logistics, reqData);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Logistics ${reqData.id ? "updated" : "created"
              } successfully`,
            data: response || {},
          });
        } catch (error) {
  ...
  // for deleting Logistics
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        for (let id of req.body.record_id) {
          let response = await crudService.destroy(Logistics, { id: id });
        }
  ...
  const get = async (req, res) => {
    try {
      ...
      response = await crudService.getAll(Logistics, {
        where: whereClause,
        attributes: [
          "id",
          "name",
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: `Logistics get successfully.`,
        data: response.rows,
        page_info: page_info,
      });
    } catch (error) {
