...
const { SupplierSchemas } = require("../../schemas/SupplierSchemas");
...
const SupplierApi = () => {
  // For create and update Supplier
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);

    validationService
      .validate(req.body, SupplierSchemas)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData.id) {
            ...
            response = await crudService.insert(Supplier, reqData);
          }
    ...
    try {
      if (req.body.record_id) {
        for (let id of req.body.record_id) {
          let response = await crudService.destroy(Supplier, { id: id });
          await crudService.destroy(SupplierBankDetails, { supplier_id: id });
        }
  ...
  const get = async (req, res) => {
    try {
      ...
      response = await crudService.getAll(Supplier, {
        where: whereClause,
        attributes: [
          "id",
          "name",
